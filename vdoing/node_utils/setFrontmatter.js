...
tags:
  - `;
      ...
      const filePath = file.filePath.replace(sourceDir, '');
      const fmData = `---
title: ${file.name}
date: ${dateStr}
permalink: ${getPermalink(filePath, file.name)}${file.filePath.indexOf('_posts') > -1 ? os.EOL + 'sidebar: auto' : ''}${cateStr}${tagsStr}
${extendFrontmatterStr}---`;
      ...
      if (!matterData.hasOwnProperty('permalink')) { // 永久链接
        const filePath = file.filePath.replace(sourceDir, '');
        matterData.permalink = getPermalink(filePath, file.name);
        hasChange = true;
      }
...
// 定义永久链接数据
function getPermalink(filePath, fileName) {
